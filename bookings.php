	...
	<script>
		$(document).ready(function(){
			...
			$("#confirm_btn").click(function(){
				...
				var choseDate=$('.clicked').nextAll("input:first").val();
		...
		function cancelBooking(id)
		{
			...
			$.ajax({
				...
				method:"POST",
				data:{coach_mail:cid,workout_id:wid,day:dayVal,time_range:rangeVal},
				error:function(xhr){
					alert(xhr.statusText);
				},
				success:function(){
...
<body>
	...
	<br><br><br>
	<input type="hidden" name="user_mail" value="<?php echo $email; ?>">
	<?php
		$sql="SELECT * FROM bookings WHERE member_mail='$email'";
		...
		if($numrow!=0)
		{
			...
			<table class="status_table" cellspacing="0">
				...
				while ($row=$Result->fetch())
				{
					...
					$workoutResult=$conn->query($sql);
					$workouts=$workoutResult->fetch();

					$coach_mail=$row['coach_mail'];
					$sql="SELECT * FROM user_details WHERE email='$coach_mail'";
					$coachResult=$conn->query($sql);
					$coach=$coachResult->fetch();
					...
					<tr>
						...
						<td <?php echo "id=day".$c; ?> ><?php echo $row['date']; ?></td>
						<td <?php echo "id=range".$c; ?>><?php echo $row['time_range']; ?></td>
						<?php
						...
						else
						{
							...
							{
								if($row['coach_cancel']=='1')
								{
									?>
									<td><span style="color: red; font-weight: bold;">Cancelled</span></td>
									<td><button type="button" class="cancelBtn" <?php echo "id=remove_btn".$c; ?> onclick="removeBooking(<?php echo $c; ?>)">Remove</button></td>
								<?php
								}
								else
								{
					...
					<tr>
						<td class="form_col">
							<label>Type of workout:</label>
								...
								<?php
								if($_GET['referer']=='workout')
								{
									$wid=$_SESSION['coaching_workout'];
									$sql="SELECT * FROM workout_plans WHERE workout_id='$wid'";
									$Result=$conn->query($sql);
									$wResult=$Result->fetch();
									$cemail=$wResult['coach_mail'];
									$sql="SELECT * FROM user_details WHERE email='$cemail'";
									$Result=$conn->query($sql);
									$cRes=$Result->fetch();
									$cname=$cRes['firstname']." ".$cRes['lastname'];
									?>
